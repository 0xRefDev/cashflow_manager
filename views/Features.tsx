  ...
  return (
    <section id="features" className="w-full min-h-[80dvh] mx-auto pb-10">
      <article className="flex flex-col gap-2 items-center text-center">
        <h1 className="text-xl lg:text-3xl font-bold leading-tight tracking-tight font-manrope text-pretty w-full text-center animate-fade-down">
          Beyond a standard tracker.{" "}
          <span className="text-[#6E9BFF]">An asset.</span>
        </h1>
        <p className="text-sm lg:text-lg text-gray-300 leading-relaxed max-w-xs lg:max-w-md text-pretty text-center mx-auto animate-fade-down animate-delay-200">
        ...
        <div className="w-full flex flex-col gap-6 lg:flex-row justify-center items-center">
          <ul className="flex flex-col items-center lg:items-start gap-6 pt-10">
            {features.map(({ icon, title, description }) => (
              <li key={title + icon} className="text-landing-primary flex flex-col items-center lg:items-start gap-3 lg:flex-row animate-fade-right animate-delay-200">
                <span>{icon}</span>
          ...
          <div className="">
            <Image src={GlobalConn} alt="Global Connect Image" className="mx-auto w-75 lg:w-160 2xl:w-3xl rounded-xl border-2 border-landing-primary/15 shadow-lg shadow-black/40 animate-fade-left" />
          </div>
        </div>
      </article>
    </section>
